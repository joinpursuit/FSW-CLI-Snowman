...
if (readlineSync.keyInYN('Do you want Play Hangman? ')) {
  // 'Y' key was pressed.
...
// //Creat an array of the allTheWords
//let lettersGuessed = lettersMatched = '';
//fuction lettersGuessed(){
  function guessIsValid(){
    return lettersGuessed.length
  }
  //  lettersGuessed.length ===1 && letter.toUpperCase() = letter.toLowerCase()}
    const wordOfObject = (word)=>{
      let wordOfObject = {}
      for (let i = 0; i < word.length; i++) wordOfObject[word[i].toLowerCase()]= true
    return wordOfObject
    }
    const wordArr = (word) =>{
