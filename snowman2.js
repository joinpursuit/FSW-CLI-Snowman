...
const wordToGuess = dictionary[Math.floor(Math.random()*dictionary.length)]
...
function keepPlaying(){
  for(let i = 0; i < stats.dashArr.length; i++){
    if(stats.dashArr === '_' && stats.guessCount > 0){
      return true
    }
  }
  return false
...
function numOfChar(){
  ...
  /*for( i = 0; i < stats.dashArr.length; i++){
    if(stats.dashArr === '_'){
      return false      
    }
  } 
