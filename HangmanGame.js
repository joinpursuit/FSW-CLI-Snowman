...
const coloring = require('chalk'); // Please install : npm install chalk
const readFromTheConsole = require('readline');
var musicPlay = require('play-sound')(opts = {}); // Please Install : npm install play-sound
...
let allTheWords = listOfWords.split(', ');
  ...
  let backgroundMusic //= musicPlay.play('Mysterious music.mp3', function (err) {
  //   if (err && !err.killed) throw err;
  // });
  let hangSoundEffect //= musicPlay.play('empty.mp3', function (err) {
  //   if (err && !err.killed) throw err;
  // });
  let victorySoundEffect //= musicPlay.play('empty.mp3', function (err) {
  //   if (err && !err.killed) throw err;
  // });
  ...
  const initPlay = () => {
    ...
    textPicture();
    backgroundMusic = musicPlay.play('Mysterious music.mp3', function (err) {
      if (err && !err.killed) throw err;
    });
  }
      ...
      if (!gameOver){
          ...
          if (userAnswer.length > 1) {
              ...
              if (!displayedWord.includes('_')) {
                  won++;
                  textPicture();
                  victorySoundEffect = musicPlay.play('Victory Sound Effect.mp3', function (err) {
                    if (err && !err.killed) throw err;
                  });
                  backgroundMusic.kill();
          ...
          if (!gameOver) {
              if (tryCounter < 6) {
                  ...
                  lose++;
                  textPicture();
                  hangSoundEffect = musicPlay.play('Hanging Sound Effect.mp3', function (err) {
                    if (err && !err.killed) throw err;
                  });
                  backgroundMusic.kill();
