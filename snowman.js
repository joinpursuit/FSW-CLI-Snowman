...
//Game Variables
//chosen dictionary word
//guessed letter
//guess count
//something to hold and show the guessed letters
//the word that is shown with the _
//
let snowman = {
  dictionaryWord:"",
  hiddenWord:"",
...
//needs to take a word from the dictionary
function randomWord(){
  return dictionaryWord = dictionary[Math.round(Math.random()*(dictionary.length))]
  
}
console.log(randomWord())
console.log(dictionaryWord)

//needs to use that word to show _ = letters in word
function showLetterSpaces(letter){
  for (let i = 0; i < snowman.dictionaryWord.length; i++) {
    if (i === 0) {
      snowman.hiddenWord = snowman.hiddenWord + '_'
    }
    else {
      snowman.hiddenWord = snowmanObject.hiddenWord + ' _'
    }
  }
  console.log(snowman.hiddenWord)
}
console.log(showLetterSpaces())
...
//if guess is invalid, show error
//if incorrect, number count decreases
//if correct, number count stays the same
//show letters guessed
function guessCount(){
  console.log(`You have ${snowman.guessesLeft} guesses remaining`)
  if (snowman.guessesLeft === 0) {
    console.log('You lose')
    console.log("The word was" + dictionaryWord)
  }
}
console.log(guessCount())


//if guess is correct, update spaces with guessed letter
function correctLetterGuess(){
...
//keep guess count visible, show at begining of game
function visibleGuessCount(){
...
//need to determine length of game by certain amount of guesses
//if word is guessed before count reaches zero display victory message
function guessedCorrect(){

}

//game ends if guesses reach zero
//show the full word with a defeat message
function outOfGuesses(){

}

//Explain directions, guess count, ask do you want to play?

function snowmanIntro(){
  console.log(`Welcome to my Snowman Game! \n The goal is to guess the word the computer has randomly chosen. \n 
  You will have 5 chances to type in a letter, but you cannot type the word.`)
  let play = readline.keyInYN("Would you like to play?")
  if(play) {
    console.log("Alright, great! Let's start!")
...
function gameLoop() {
  while(snowman.guessesLeft !== 0)
  gameOver()
}
function startGame(){

}
//Ask do you want to play again, if not terminates game.
function gameOver(){
  process.exit
}
