...
function getRandomIndex(min, max) {
  ...
  // there is an way of doing this with less code, by using: 
  // let randomWord = dictionary[Math.floor(Math.random() * dictionary.length)]
  // but I wanted to try something different, a function that I can use on a variable
}

function generateSecretWord(random, secret) {
...
function isLetterInWord(newLetter, stats, random, secret) {
  let guessCorrect = false
  for (let i = 0; i < random.length; i++) {
    if (random[i] === newLetter) {
      secret[i] = newLetter
      guessCorrect = true
    }
  }
