...
const playLoop = () =>{
  ...
  while(underscore.join("") !== stats.randomEl){
    guess(underscore)
  }
}
...
const guess = (underscore) =>{
  ...
  if(underscore.join("") === stats.randomEl){
    console.log(stats.randomEl)
    console.log("You got it! Congrats! It took you " + totalGuesses + " guesses.")

    playAgain()
  }else if(stats.guessesLeft === 0){
    console.log(`You LOST. The word was "${stats.randomEl}". You used ${stats.allGuesses.length} tries. Better luck next time.`)
    playAgain()
  }
...
function getValidLetterGuess() {
  function guessIsValid(letterGuess) {
    return letterGuess.length === 1 && letterGuess.toUpperCase() != letterGuess.toLowerCase() && !stats.allGuesses.includes(letterGuess)
  }
  let letterGuess = ""
  while (!letterGuess) {
  ...
  return letterGuess.toLowerCase()
}
