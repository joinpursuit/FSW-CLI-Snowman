...
const gameLoop = () => {
  while(stats.guessesRemaining > 0 || stats.board.includes(`_`)){
    console.clear()
    console.log(chalk.yellowBright`${stats[`board`].join(` `)}`)
    console.log(chalk.greenBright`\nGuessed letters: ${stats[`previouslyGuessed`]}`)
    console.log(chalk.cyanBright`Guesses remaining: ${stats[`guessesRemaining`]}`)
    
    guess = getValidLetterGuess()

    const duplicateGuess = () => {
      if(stats.previouslyGuessed.includes(guess)){
        console.log(chalk.redBright`You already guessed that letter! Please guess again.`)
        gameLoop()
      }
    }
    duplicateGuess()
...
const winOrLose = () => {
  if(stats.guessesRemaining === 0){
    ...
    console.log(chalk.redBright`The word was ${stats.word}`)
    process.exit()
  } else if (!stats.board.includes(`_`)){
    console.log(chalk.greenBright`\nCongrats! You won in ${stats.guessCounter} guesses!`)
    console.log(chalk.greenBright`The word was ${stats.word}`)
    process.exit()
  }
}
...
function getValidLetterGuess() {
  ...
  let letter = ""
  while (!letter) {
    let input = readline.question(chalk.magentaBright`\nPlease enter your guess: `)
      if (guessIsValid(input)) {
        letter = input
      } else {
        console.log(chalk.redBright`Please enter a valid letter`)
      }
  }
