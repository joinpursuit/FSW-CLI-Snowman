...
const reprompt = ` That's an invalid enter`
...
const loop = ()=>{
  console.log(`Hello,\n Welcome to the world: WORD SOUP`)
  pause(0.9)
...
const letterGuessed = () => {
  // tell the user how many guess the have left
  ...
  // tell the user that pick a letter
  guessedLetter = getValidLetterGuess()
  // update the guesed left
  // telling the user if the letter are correct or not

  
  
  if(letterArray.includes(guessedLetter)){
    rightGuess(letterArray, guessedLetter)
  } else if(userStatus.guessedLetters.includes(guessedLetter)){
...
const rightGuess = (arr, v) => {
  ...
  ind.forEach((elem) => {
    answer.splice(elem, 1, guessedLetter)
  })

  answerUser = answer.join(" ")
  userStatus.guessedLetters.push(guessedLetter)
...
const wrongguess = () => {
  ...
  pause(0.3)
  console.log(` but don't give up ${userName} you still have time`)
  pause(0.3)
  console.log(`  " Please try Again " `)
  console.log(answerUser)
  if(userStatus.guessedNum === 0){
    console.log(`You did not got the word`)
    console.log(`This was the word ` + `${currentWord.toUpperCase()}`)
    quitGame()
  }else{
    letterGuessed()
  }
