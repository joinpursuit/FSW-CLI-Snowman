...
const guessTheLetters = () => {
  // lets you guess the letter and tell you how many guesses you have left.
  if (numberOfGuesses.maxGuesses === 1) {
    console.log(`You now have ${numberOfGuesses.maxGuesses} wrong guess left.`);
  } else {
    console.log(
      `You now have ${numberOfGuesses.maxGuesses} wrong guesses left.`
      );
    }
  ...
  //directs you depending on your guess
  if (ranWordArray.includes(guess)) {
    correctGuessFunction(ranWordArray, guess);
  } else {
    wrongGuessFunction(guess);
  }
};
  const wrongGuessFunction = (guess) => {
    //if you enter the wrong guess or you lose
    numberOfGuesses.guessedLetters.push(guess);
    let guessedLettersJoin = numberOfGuesses.guessedLetters.join(" ");
...
const correctGuessFunction = (ranWordArray, guess) => {
  //adds letter to the random word if correct and give the winning message
  let indexWord = [];
  ...
  if (showAnswer.indexOf("_") < 0) {
    console.log(
      `Congratulations ${nameOfPlayer}!! you guessed it in ${totalOfGuesses} guesses.`
    );
    process.exit()
  } else {
    guessTheLetters();
