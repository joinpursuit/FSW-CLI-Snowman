...
const letterGuess = () => {
  if (stats.guesses === 1){
    console.log(chalk`\n{cyan You have {red ${stats.guesses}} incorrect guess left.}\n`)
  } else {
  ...
  if (randomWordArr.includes(playerGuess)){
    correctGuess(randomWordArr, playerGuess)
  } else {
    wrongGuess();
  }
}
...
const correctGuess = (array, value) => {
  ...
  indexes.forEach((el) => {
    reveal.splice(el, 1, playerGuess)
  })
...
const wrongGuess = () => {
  ...
  console.log(revealWord)
  if (stats.guesses === 0){
    console.log(chalk`\n{red You lose! The correct word was {magenta ${randomWordStr}}.}`)
    process.exit();
  } else {
    letterGuess()
  }
...
function getValidLetterGuess() {
  ...
  while (!letter) {
    ...
    if (guessIsValid(input)) {
      ...
      console.log(chalk`\n{rgb(255,165,0) Please enter a valid letter.}\n`)
    }
  }
