...
for (let i = 0; i < obj.randomWord.length; i++) {
  obj.answerArray[i] = '_'
}
...
const gameLoop = () => {
  console.log(obj.randomWord)


  for (let i = 0; i <= obj.randomWord.length; i++) {
    if (obj.randomWord[i] === obj.guess) {
      obj.answerArray[i] = obj.guess
    }
  }
  if (obj.guessedLetterArr.includes(obj.guess)){
    console.log('You guessed that letter already try again!!!\n')
    getValidLetterGuess()
  }
  else if (!obj.randomWord.includes(obj.guess)) {
    obj.missedGuesses++
  }
  guessCount()
  g = obj.answerArray.join('')
  if (obj.randomWord !== g && obj.numOfGuesses !== 0) {
    getValidLetterGuess();
    gameLoop();

  } else if (obj.numOfGuesses === 0) {
    console.log('Sorry , you ran out of guesses!')
    console.log(`\nThe word was: ${obj.randomWord}`)
  } else {
    winner();
  }
}
...
function guessCount() {
  if (!obj.answerArray.includes(obj.guess)) {
    obj.numOfGuesses--
  }
  q = obj.answerArray.join(' ');
  console.log(q)
