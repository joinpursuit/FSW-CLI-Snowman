      ...
      while (!letter) {
        ...
        if (guessIsValid(input)) {
          ...
          if(status.countLetter === 1 && status.answer !== word){
            console.log(`\n ${status.wordProgress }          ${ status.guessedMade} \n`)
            console.log(`${endingGame.lose}`)
            quitGame()
          }else if(status.answer === word){
            console.log(`${word}`)
            console.log(`${endingGame.win}`)
            console.log(`You get the word in ` + status.countTotalGuessed + ` guesses`)
            startGame()
          }
...
const quitGame = () => {
  console.log(chalk.yellow(`      Comeback soon to play`))
  process.exit()
}
