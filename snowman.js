...
const buildBoard = () => {
  for (let i = 0; i < data.randomWord.length; i++){
      data.hiddenWord.push('_')
  }
  gameLoop()
}
...
const ifLetterInWord = (letter) => {
  for (let i = 0; i < data.randomWord.length; i++) {
    if (data.randomWord[i] === letter) {
      data.hiddenWord[i] = letter;
    }
  }
  if (!data.hiddenWord.includes(letter)) {
    data.tracking.count--;
  }
  if (data.hiddenWord.includes("_")) {
    data.gameNotFinished = true;
  } else {
    data.gameNotFinished = false;
  }
};
  
const gameLoop = () => {
  console.clear();
  while (data.gameNotFinished) {
    console.log(data.hiddenWord.join(" "));
    console.log(`You have ${data.tracking.count} guesses left.`);
    console.log("Letters guessed: " + data.tracking.usedLetters.join(" "));
    letter = getValidLetterGuess();
    if (data.tracking.usedLetters.includes(letter)) {
      console.clear();
      console.log("Letter already guessed");
    } else {
      console.clear();
      ifLetterInWord(letter);
      data.tracking.usedLetters.push(letter);
...
const amountGuessed = () => { 
  let amount = data.tracking.usedLetters.length
  console.log(`Congrats! You won! You guessed the word "${data.randomWord}" in ${amount} guesses.`)
}

const lost = () => {
    console.clear()
    console.log("Sorry, you ran out of guesses.")
    console.log(`The correct word is "${data.randomWord}". Better luck next time.`)
    restartGame()
}
...
let restartGame = () =>{
  if(readLineSync.keyInYNStrict(`Do you want to play again?`)){
    console.clear()
    data.hiddenWord = []
    data.gameNotFinished = [] 
    data.tracking = {count:8, usedLetters:[]}
    data.randomWord = dictionary[Math.floor(Math.random() * dictionary.length)]
    buildBoard()
  }else{
    endGame()
  }
