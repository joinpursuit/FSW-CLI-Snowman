...
const startGame = () => {
  console.log(
    "Here are the instructions: You have " + playerObj.guessCount + " guesses"
  );
  hideWord();
  gameLoop();
};
...
const gameLoop = () => {
  ...
  while (playerObj.guessesLeft > 0) {
    ...
    if (chosenWord.split("").includes(guessLetter)) {
      for (let i = 0; i < chosenWord.length; i++) {
        if (chosenWord[i] === guessLetter) {
          hiddenWordArr[i * 2] = guessLetter;
        }
      }
      playerObj.guessesCorrect += 1;
      if (playerObj.guessesCorrect === chosenWord.length) {
        console.clear()
        console.log("You won!")
      }
    ...
    console.clear()
    console.log(playerObj.displayWord);
    console.log("Guessed letters: " + guessHistory)
